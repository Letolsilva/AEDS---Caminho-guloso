...
- le_arquivo: Ela se inicia com uma variável booleana como true, ela é responsável por manter a leitura do arquivo da primeira linha e coluna de forma correta, pois a primeira posição já foi definida na main (main - linha 25). Em seguida, existem dois loops, um de linhas (i) e outro de colunas (j), que acontecerão até o tamanho da matriz (num). Dentro dele, se encontra o comando  if no qual lê os dados do arquivo e recebe uma condição que é verdadeira enquanto o programa ler valores diferentes de final de arquivo. (linha 11).<br>
